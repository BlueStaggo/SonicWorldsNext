...
func _physics_process(delta):
	...
	if timerActive and timer > 0:
			...
			if wrapf(timer,0,8.0/60.0) < wrapf(timer-delta,0,8.0/60.0):
				...
				# set animal position, starting from -28 checked the x position and increasing by 8 per animal
	...
	# after flickes are gone, set stage clear to 3 (2's for running unchecked screen, see the goal post)
