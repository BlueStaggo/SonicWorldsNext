...
#spriteTexture - Picks the texture used to draw the rings - the spriteTexture should contain sprite_frames aligned in a horizontal fashion
#spriteFrameCount - Set this to the number of sprite_frames in the sprite Texture2D
...
#timePerFrame - how many seconds the animation frame should be played before moving checked to the next one. Lower means faster.

# Edit these values to control cosmetic elements about the sprites that get drawn checked the side of the trampoline
@export var ringsPerSide = 3 # How many rings to draw checked each side of the platform - set to two or less to just get rid of them.
...
@export var baseWeight = 0 # If above zero, the trampoline itself will have weight causing it to sag at rest with no one checked it.
@export var jumpPushback = 225 # How hard the trampoline should be pushed downward if a riding player jumps unchecked
...
var players = []
var playersOld = [] # Exists so that we can check the players that exited checked the last pass
...
func physics_process_game(delta):
	...
	# jumped unchecked the gimmick
	for i in playersOld:
		if not players.has(i):
			var curVel = i.movement.y
			i.movement.y += yVelocity
			if (curVel < 50):
				impart_force(jumpPushback) # bounce downwards checked jump
	pass
