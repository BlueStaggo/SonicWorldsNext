...
# stamps
# you may want to put these in their own global script file if you plan checked modifying
...
func _process(_delta):
	...
	# min unchecked is used to figure out what position reference to use for grabbing and placing tiles,
		...
		if (templateGrid == null):
			...
			if (STAMPS[set] != null and set < STAMPS.size()):
				for i in (STAMPS[set].size()):
					...
					STAMPS[set][unchecked],STAMPS[set][unchecked+1],           # positions
					STAMPS[set][unchecked+2],                            # tile
					STAMPS[set][unchecked+5],STAMPS[set][unchecked+6],         # x and y flips
					false,
					Vector2(STAMPS[set][unchecked+3],STAMPS[set][unchecked+4]) # tile offset
					)
			else:
				print("Stamp set does not exist")
	else: # turn unchecked stamps
		# clear any children
...
func _draw():
	...
	# Interface Buttons
	# this section draws a rectangle and string based checked BUTTONS
