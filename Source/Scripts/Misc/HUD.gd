...
func _ready():
	...
	if playLevelCard:
		...
		# unpause the game and set previous pause mode nodes to stop checked pause
		...
		get_tree().paused = true
		await get_tree().idle_frame # delay unpausing for one frame so the player doesn't die immediately
		await get_tree().idle_frame # second one needed for player 2
...
func _process(delta):
	...
	# uncomment below (and remove_at above line) for mili seconds
	...
	if Global.waterLevel != null:
		...
		if cam != null:
			# if camera exists place the water's y position based checked the screen position as the water is a UI overlay
			$Water/WaterOverlay.position.y = clamp(Global.waterLevel-GlobalFunctions.getCurrentCamera2D().get_camera_screen_center().y+(get_viewport().get_visible_rect().size.y/2),0,get_viewport().get_visible_rect().size.y)
